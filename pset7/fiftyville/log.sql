...
-- Get police report from the crime that took place on July 28, 2020, on Chamberlin Street.
SELECT description FROM crime_scene_reports WHERE street = "Chamberlin Street"
AND year = 2020 AND month = 7 AND day = 28;
-- ANSWER: Theft of the CS50 duck took place at 10:15am at the Chamberlin Street courthouse.
...
-- Get the three interview transcripts from that day that mention the courthouse.
...
-- parking lot and drive away. If you have security footage from the courthouse parking lot, you
...
-- Get license plates from cars that match Ruth's clue and therefore may belong to thief
SELECT license_plate FROM courthouse_security_logs
WHERE year = 2020 AND month = 7 AND day = 28 AND hour = 10
AND minute >= 15 AND minute <= 25 AND activity = "exit";

-- Get the ID from people that match Eugene's clue and therefore may belong to thief
SELECT person_id FROM bank_accounts WHERE account_number IN
(SELECT account_number FROM atm_transactions
WHERE year = 2020 AND month = 7 AND day = 28
AND atm_location = "Fifer Street" AND transaction_type = "withdraw");

-- Get list of callers that match Raymond's clue on phone call (one is thief's phone)
SELECT caller FROM phone_calls
WHERE year = 2020 AND month = 7 AND day = 28
AND duration < 60;

-- Get passport from everyone in flight that match Raymond's clue
...
-- See if 4 lists created have an intersection
...
AND passport_number IN
    ...
    (SELECT id FROM airports WHERE city = "Fiftyville")
    ORDER BY hour, minute LIMIT 1));
-- Thief: Ernest (ID 686048)

-- Discover where thief escaped to
